...
if(isset($_GET["shortcode"])){
    ...
    function ux_shortcodePreview($content = false) {
        ob_start(); ?>
        <?php include('html/'.$_GET["shortcode"].'.php'); ?>        
        <?php 
        $content = ob_get_contents();
        ob_end_clean();
        return $content;
    }
    add_filter('the_content', 'ux_shortcodePreview');
}

/* Preview Shortcode  */
function uxPreview($atts, $content = null) {
    if(!function_exists('ux_shortcodeDisplay')) {
    function ux_shortcodeDisplay(){
            echo '<style>.shortcode-open{display:block!important;}.ux-shortcode{position:relative}.ux-shortcode .get-shortcode{z-index:999;position:absolute;top:0;left:0;right:0;opacity:0;padding:0px;}.ux-shortcode:hover .get-shortcode{opacity:1}   html{zoom:0}.get-shortcode textarea{height:200px;display:none;} .get-shortcode span{margin-left:10px;}.get-shortcode{margin-top:10px;}body{background:none!important;}#sw-open,#masthead,.top-divider{display:none}.footer-wrapper{display:none;}</style>'; ?>
            <script>/*
 * jQuery Notebook 0.5
 *
 * Copyright (c) 2014
    ...
    $.fn.notebook.defaults = {
        ...
        modifiers: ['bold', 'italic', 'underline', 'h1', 'h2', 'ol', 'ul', 'anchor']
            ...
            jQuery( document ).ready(function($) {
                 $('.get-shortcode').each(function(){
                    $(this).click(function(){
                        $(this).find('span').html('Copy and paste this text into your WordPress Content editor.');
                        var text = $(this).find('textarea').val();
                        text = text.replace(/https?:\/\/.*?\.(?:png|jpg|jpeg|gif)/ig, "http://imageurl");
                        $(this).find('textarea').val(text);
                        $(this).find('textarea').toggleClass('shortcode-open');
                    });
                 });
                });
            </script> 
...
.jquery-notebook input[type=text] {
    ...
    add_action('wp_head', 'ux_shortcodeDisplay');
    }
    ob_start(); ?>
    <div class="ux-shortcode">
    <?php echo do_shortcode($content); ?>
    <div class="get-shortcode">
    <a href="#" clasS="button">Get shortcode</a>
...
add_shortcode('ux_preview', 'uxPreview');
...
function add_toolbar_items(){
    global  $wp_admin_bar; 
     ...
     $wp_admin_bar->add_menu( array(
        ...
        'parent' => 'ux_s',
        'title' => 'Page Layouts',
        'href'  => $link = get_site_url().'/?shortcode=layouts&TB_iframe&width=990&height=500',
    ...
    $wp_admin_bar->add_menu( array(
        'id'    => 'ux_s_row',
        'parent' => 'ux_s',
        'title' => 'Row / Columns / Sections',
        'href'  => $link = get_site_url().'/?shortcode=row&TB_iframe&width=990&height=500',

        'meta'  => array(
            'title' => __('Row / Column / Sections'),
            'class' => 'thickbox'
        ),
    ));
        ...
        'parent' => 'ux_s',
        'title' => 'UX Sliders',
        'href'  =>  $link = get_site_url().'/?shortcode=sliders&TB_iframe&width=1150&height=500',
        'meta'  => array(
            'title' => __('UX Sliders'),
            'class' => 'thickbox'
        ),
    ...
    $wp_admin_bar->add_menu( array(
        ...
        'parent' => 'ux_s',
        'title' => 'UX Banners',
        'href'  =>  $link = get_site_url().'/?shortcode=banners&TB_iframe&width=1150&height=500',
        'meta'  => array(
            'title' => __('UX Banners'),
            'class' => 'thickbox'
        ),
    ...
    $wp_admin_bar->add_menu( array(
        'id'    => 'ux_s_banner_grids',
        'parent' => 'ux_s',
        'title' => 'Banner Grids',
        'href'  =>  $link = get_site_url().'/?shortcode=grid&TB_iframe&width=1150&height=500',
        'meta'  => array(
            'title' => __('Banner grids'),
            'class' => 'thickbox'
        ),
    ));

}
add_action( 'wp_before_admin_bar_render', 'add_toolbar_items',999);


function add_shortcode_lightbox(){ ?>
<?php add_thickbox(); ?>
<style>
#wp-admin-bar-ux_s{background-color: #5282c4!important;}
#TB_window.shortcode_iframe,#TB_iframeContent{width:1200px!important;}
#TB_window.shortcode_iframe{margin-left:-600px!important;};
</style>
<script>
/* LIGHBOX POPUP SCRIPT */
jQuery( document ).ready(function($) {
  $('#wp-admin-bar-ux_s').find('.thickbox >a').each(function(){
    $(this).addClass('thickbox');
    $(this).click(function(){
        $('.menupop').removeClass('hover'); 
        event.preventDefault();
        setTimeout(function() {
              $('#TB_window, #TB_iframeContent').addClass('shortcode_iframe');
        }, 10);
    });
});
});
...
add_action( 'admin_footer', 'add_shortcode_lightbox' );
}
