...
<form id="yith-wcwl-form" action="<?php echo esc_url( $yith_wcwl->get_wishlist_url() ) ?>" method="post">
    ...
    <table class="shop_table cart wishlist_table" cellspacing="0">
        ...
        <tr>
            ...
            <th class="product-name"><span class="nobr"><?php _e( '商品名稱', 'yit' ) ?></span></th>
            <?php if( get_option( 'yith_wcwl_price_show' ) == 'yes' ) : ?><th class="product-price"><span class="nobr"><?php _e( '價格', 'yit' ) ?></span></th><?php endif ?>
            <?php if( get_option( 'yith_wcwl_stock_show' ) == 'yes' ) : ?><th><span class="nobr"><?php _e( '庫存狀況', 'yit' ) ?></span></th><?php endif ?>
        ...
        if( count( $wishlist ) > 0 ) :
                        ...
                        <?php if( get_option( 'yith_wcwl_price_show' ) == 'yes' ) : ?>
                            <td class="product-price">
                                ...
                                if( $product_obj->price != '0' ) {
                                    ...
                                    echo apply_filters( 'yith_free_text', __( '免費!', 'yit' ) );
                                }
                                ?>
                            </td>
                        <?php endif ?>
                        <?php if( get_option( 'yith_wcwl_stock_show' ) == 'yes' ) : ?>
                            <td class="product-stock-status">
                                ...
                                if( $stock_status == 'out-of-stock' ) {
                                    $stock_status = "Out";
                                    echo '<span class="wishlist-out-of-stock">' . __( '缺貨', 'yit' ) . '</span>';
                                } else {
                                    $stock_status = "In";
                                    echo '<span class="wishlist-in-stock">' . __( '有庫存', 'yit' ) . '</span>';
                                }
                                ?>
                            </td>
                        <?php endif ?>
            ...
            <tr>
                <td colspan="6" class="wishlist-empty"><?php _e( '您尚未收藏任何商品!', 'yit' ) ?></td>
            </tr>
        <?php
        endif;
